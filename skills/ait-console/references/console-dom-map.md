...
- **실측(issue #1, 2026-06-11) — 게이트 통과 시 정상 폼**: 행별 **"검토 요청"** 버튼 → **"검토 요청하기" 모달**(출시 노트 textarea, placeholder "1. 프로필 화면에서 버튼 클릭 시 반응이 없던 문제 수정") → **"확인"/"검토 요청하기" 2버튼**. **v3.8.0부터 `submit-review`는 MANUAL이다** — 스크립트는 안내만 출력하고 자동 제출하지 않는다(자동 제출 구현·API 자동 캡처는 제거됨). 위 DOM 경로는 **사용자가 콘솔에서 직접 따라가는 절차**이자 에이전트의 안내 문구 근거다. (참고: 기존 출시 앱들의 bundles `releaseNote`가 이 폼의 산출물.) 작성 준비한 출시노트(미제출): "오늘의 소비 뽑기" 소개 4줄 — `dumps-write/review-result.json`에 보관.
- **검토 요청 취소 실측(issue #1, 2026-06-11)**: 검토중 버전 행에 **"요청 취소"** 버튼 존재 — 클릭 시 상태 **"검토 중" → "요청 취소됨"** 전이. **v3.8.0부터 `cancel-review`는 MANUAL이다**(파괴적 동작) — 스크립트는 이 경로를 안내만 하고 클릭하지 않는다.
