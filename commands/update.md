...
7. **Phase C'** — ait-console 스킬: `upload`(테스트 버전)까지 자동 수행(register는 파이프라인 자동 단계 아님). 각 단계 결과를 보고서 §콘솔 테스트에 기록. **이후 테스트 발송·앱 정보 입력·검토 요청·출시는 자동 수행하지 않으며**, 보고서 §사용자가 해야 할 것 체크리스트에 콘솔 수동 절차를 안내한다. `--auto` 모드도 동일하게 `upload` 단계 한정.
...
update 실행 시 **광고 적용 여부를 점검**한다 — 광고(최소 배너)가 없거나 부족하면 광고 추가를 변경 후보로 **제안**한다(광고는 수익화 수단이지 검수 필수 항목이 아니다 — 도입 여부는 사용자에게 확인하고, 미포함 결정이면 그대로 진행한다). **광고성 푸시알림(스마트발송)·프로모션도 적극 추천한다** — 세그먼트·캠페인·문구·ROI(`ait-promotion-reward` "프로모션 ROI 검토" 절) 설계를 APP-SPEC.md에 산출하고, 콘솔 등록·신청 실행은 사용자가 직접 수행한다 (pipeline §사용자 직접 수행 영역과 적극 추천 의무 참조).
