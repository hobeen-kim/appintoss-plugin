...
| SDK 표기(문서·코드) | 콘솔 표기(광고 그룹 생성 화면) | 생김새 | 높이·배치 | 테스트 ID |
|------|------|------|------|-----------|
| 전면형 | (콘솔 표기 미확인 — 등록 시 실측 필요) | 화면 전체를 덮음 | **사용자 명시 액션(버튼) 직후 등 예측 가능한 시점에만 노출** — 로딩/인트로 등 일시 화면 노출 금지 | `ait-ad-test-interstitial-id` |
| 보상형(리워드) | (콘솔 표기 미확인 — 등록 시 실측 필요) | 시청형 | 사용자가 선택해서 시청 후 보상 수령 | `ait-ad-test-rewarded-id` |
| 배너 리스트형 | **문구 강조** | 아이콘 + 한 줄 제목 + 부제 + `AD` | 96px 높이 고정. 콘텐츠 하단/리스트 중간 | `ait-ad-test-banner-id` |
| 배너 피드형 | **이미지 강조** | 아이콘 + 제목 + 큰 이미지 카드 + CTA 버튼 | 410px 높이 고정. 피드 사이 | `ait-ad-test-native-image-id` |

> **사용자에게 배너를 제안할 때는 콘솔 표기로 말한다.** 콘솔 광고 그룹 생성 화면의 선택지는 `문구 강조`/`이미지 강조` 두 가지이며 SDK 이름(`배너 리스트형`/`배너 피드형`)과 글자가 전혀 겹치지 않는다. "배너 광고를 넣겠습니다"로만 보고하면 사용자는 문구형인지 이미지형인지 알 수 없다 — **어느 쪽인지와 생김새를 함께 밝힌다.**
> 전면형·보상형의 콘솔 표기는 미확인 상태다. 해당 광고를 등록할 때 콘솔 화면을 확인해 이 표를 갱신하라.
...
## 광고 유형 선택 — eCPM 실측 범위와 의사결정 (웨비나 실측)

> 출처: 앱인토스 공식 웨비나 2026-08-21(개발자 실사례). **절대치가 아니다** — 기간·지면·앱 성격에 따라 크게 변동한다.
> 유형 간 **상대 격차**와 **판단 절차**를 참고선으로 쓴다.

| 유형 | eCPM 실측 범위 | UX 부담 | 성립 조건 |
|---|---|---|---|
| 문구형 배너(배너 리스트형 / 콘솔 「문구 강조」) | **2,000원 초반** | 가장 낮음 | 상시 노출 지면 확보 |
| 전면형 | 배너와 리워드 사이 | **5초 후 스킵** — 유튜브 5초 스킵 수준으로 수용됨 | 사용자 명시 액션 직후 |
| 리워드 | **약 3만 원** | **30초 이상 시청** | 유저가 **보상에 납득**해야 완주한다 |

**전면형 → 리워드 전환 의사결정 (실제 밟은 순서)**

1. **전면형만으로 시작** — 뒤에 결제가 있으면 UX 훼손을 먼저 경계한다
2. **실험하고 지표를 직접 측정** — 로그·대시보드가 있어야 가능하다(`ait-analytics`)
3. **"유저가 이 보상에 납득하는가"** 관점에서 자기 앱이 조건을 충족하는지 판단
4. 유저가 리워드 광고를 **끝까지 본다**는 것이 확인되면 → 리워드로 전환해 eCPM을 크게 올린다

> 시도하지 않으면 **약 15배 차이(2,000원대 ↔ 3만원대)** 를 놓친다.
> 다만 보상 설계가 약한 앱에서 리워드를 강행하면 중도 이탈로 노출만 소모된다 — **먼저 전면형, 지표로 검증 후 전환**이 안전한 순서다.
