...
11. **Phase C** — ait-console 스킬: `upload`(테스트 버전)까지 자동 수행. 각 단계 결과를 보고서 §콘솔 테스트에 기록. **콘솔 앱 생성(register)은 사용자 직접 수행** — 보고서 §🙋 사용자가 해야 할 것 체크리스트에 안내. **이후 테스트 발송·앱 정보 입력·검토 요청·출시는 자동 수행하지 않으며**, 보고서 §사용자가 해야 할 것 체크리스트에 콘솔 수동 절차를 안내한다. `--auto` 모드도 동일하게 `upload` 단계 한정.
...
**트리거 게이트**: 화면 변경 또는 버전 상승이 발생한 지시는 `docs/REPORT-v{version}.md` 미발행 시 완료로 간주하지 않는다.
...
**출시는 별도다.** 사용자가 "출시해라"라고 하면 `ait-console release-status`로 현재 상태를 확인한 뒤, 검토 요청 및 출시는 **콘솔에서 직접 수행하는 절차를 안내**한다. `release-watch`는 상태 감지·알림 전용이며 검토 요청·출시를 자동 수행하거나 `--confirm` 후 수행하지 않는다. 파이프라인·`--auto` 모드에서 출시를 자동 수행하지 않는다.
