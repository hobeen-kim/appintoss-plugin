...
> **사용자 직접 수행 영역**: 프로모션 등록·신청·비즈월렛 충전은 사용자가 콘솔에서 직접 수행한다 — 에이전트는 ROI 검토·코드 패턴(promotionCode 참조 등)·가이드 제공까지만 하고, 파이프라인에서 프로모션 도입을 자동 제안하지도 않는다(사용자 명시 요청 시에만 가이드). (사용자 지시 2026-06-11)
