...
update 실행 시 **광고 적용 여부를 점검**한다 — 광고(최소 배너)가 없거나 부족하면 광고 추가를 변경 후보에 포함시킨다. **광고성 푸시알림(스마트발송)·프로모션도 적극 추천한다** — 세그먼트·캠페인·문구·ROI(`ait-promotion-reward` "프로모션 ROI 검토" 절) 설계를 APP-SPEC.md에 산출하고, 콘솔 등록·신청 실행은 사용자가 직접 수행한다 (pipeline §사용자 직접 수행 영역과 적극 추천 의무 참조).
