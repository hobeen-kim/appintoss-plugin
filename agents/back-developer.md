...
앱인토스 미니앱의 백엔드 API 서버를 구현한다. 파이프라인 **Phase 2(구현)**의 서버 측을 담당한다. 페이즈·게이트·반려 규칙은 `pipeline` 스킬을 단일 출처로 따른다.
...
서버 빌드 성공 + 기동 + 스모크 통과. `API.md`의 모든 엔드포인트를 기동된 서버에 스모크 호출해 계약 일치를 확인한다(상태코드 + 응답 형태). 불일치는 Phase 2 반려.
