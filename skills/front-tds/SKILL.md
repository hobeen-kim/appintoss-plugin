...
## TDS 피그마 파일을 AI 디자인 도구에 이식하기 (권장 세팅)

디자인 시안을 AI로 뽑을 때, **TDS를 AI에 먼저 학습시켜 두면** 이후 모든 산출물이 토스 규격으로 나온다.

1. **앱인토스 개발자 센터**(웹) → **「디자인 도구」** 검색
2. **TDS zip 다운로드** → 압축 해제하면 **`.fig`(피그마 파일)** 이 나온다 — 버튼·라벨·세그먼트 컨트롤 등 컴포넌트 수록
3. 그 `.fig`를 사용하는 **AI 디자인 도구(예: Claude 디자인)에 통째로 등록**
4. 도구가 **20~30분간 자체 학습** → TDS 체계를 이해하고 자기 환경에 반영
5. 이후 어떤 화면을 시켜도 **버튼 곡률·두께·배경색까지 TDS 기반**으로 산출된다

> 필수 절차는 아니지만, 디자이너 없이 "토스러운" 화면을 얻는 가장 빠른 경로다.
> 단, **최종 구현은 반드시 실제 TDS 컴포넌트(`@toss/tds-mobile`)로** 한다 — 시안을 커스텀 HTML/CSS로 옮기면 검수에서 반려된다.
> (출처: 앱인토스 공식 웨비나 2026-08-21 실사례)
