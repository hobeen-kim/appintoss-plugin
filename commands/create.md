...
11. **Phase C** — ait-console 스킬: `register`(미등록 시) → `upload-assets` → `upload`(테스트 버전)까지 자동 수행. 각 단계 결과를 보고서 §콘솔 테스트에 기록. **이후 테스트 발송·앱 정보 입력·검토 요청·출시는 자동 수행하지 않으며**, 보고서 §사용자가 해야 할 것 체크리스트에 콘솔 수동 절차를 안내한다. `--auto` 모드도 동일하게 `upload` 단계 한정.
